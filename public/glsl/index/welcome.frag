...
vec3 matrix(vec2 _coord) {
    ...
    float colHeight = sin(float(x)) * 50.0 + 60.0;
    int y = int(mod(mod(float((int(resolution.y) - coord.y) / int(fontSize)), 50.0) - time * 10.0, colHeight));
    int charPixelX = int(mod(float(coord.x), fontSize) / (fontSize / 8.0));
    int charPixelY = int(mod(float(coord.y), fontSize) / (fontSize / 8.0));
   
    bool isFilled = char(string(y), charPixelX, charPixelY);
    vec3 col = isFilled ? vec3(float(y > 30 + int(3.0 * sin(float(x)))) * 0.4 + 0.6) : vec3(0.0); // black if filled, white otherwise
    return col;
}
